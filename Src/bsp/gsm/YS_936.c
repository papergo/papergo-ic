...
#define UGBKADDR	980*4096

/* 私有变量 ------------------------------------------------------------------*/
/* 扩展变量 ------------------------------------------------------------------*/
/* 私有函数原形 --------------------------------------------------------------*/
/* 函数体 --------------------------------------------------------------------*/
uint16_t ff_convert (	/* Converted code, 0 means conversion error */
...
)
{
	...
	else 
	{
		...
		{
			i = li + (hi - li) / 2;	
			SPI_FLASH_BufferRead((uint8_t *)&t,gbk2uni_offset+i*4+UGBKADDR,4);//读出4个字节
			if (src == t[0]) break;
			if (src > t[0])li = i;  
			else hi = i;    
		}
